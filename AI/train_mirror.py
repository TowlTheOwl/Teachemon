...
class MirrorOpponentWrapper(gym.Env):
...
def mask_fn(env: MirrorOpponentWrapper) -> np.ndarray:
...
def train_agent():
    ...
    mirror_opponent = MirrorOpponent(model=None)

    base_env = BattleGymEnv()
    env = MirrorOpponentWrapper(base_env, mirror_opponent)
